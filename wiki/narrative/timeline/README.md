...
Chronological event pages — the temporal index of the story world. Boundary events
(`is_boundary: true`) serve as anchors for the `valid_from`/`valid_until` system used
across all other page types.

> **Two timelines:** This directory tracks *story-world* events. Real-world source
> provenance (ingestion dates, publication dates) lives in `wiki/knowledge/timeline/`.
...
- Related to: `wiki/narrative/chapters/` — chapters use `timeline_start:` and `timeline_end:` to reference events here
- Related to: `wiki/reader_state/reader-model/` — reader-state pages are anchored to timeline positions
